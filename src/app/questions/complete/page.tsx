      ...
      <div className='flex-1 w-full px-6 py-8 flex flex-col items-center justify-center text-center bg-gradient-to-b from-gray-900 to-black'>
        <div className='w-full max-w-2xl mx-auto'>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className='space-y-8'>
            ...
            {/* 응답 검색 링크 추가 */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }} className='text-center mt-8'>
              <p className='text-gray-400 font-["Pretendard"] mb-3'>내 답변을 나중에 확인하고 싶다면:</p>
              <motion.a
                href='/answer'
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className='inline-block px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-all duration-200 border border-white/10 font-["Pretendard"]'
              >
                내 답변 확인하기
              </motion.a>
            </motion.div>
